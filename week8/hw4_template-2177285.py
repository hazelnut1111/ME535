## Student ID :2177285 Name:Han-Yi Lin Date:2023/03/02

## Study group : student ID :2273420 Name:Zelin Yang
...
def single_step(x0,x1,dx,dt):
	...
	c = 1
	lam=np.power(c*dt/dx,2)
	x2 = np.zeros_like(x1)

	for i in range(1, len(x1)-1):
		x2[i] = 2*x1[i] -x0[i] + lam*(x1[i+1] - 2*x1[i] + x1[i-1])
...
def writen_response_p2():
	...
	return """ANSWER:From the test code, when the CFL number is slightly above 1 (s = 1.001), the solution appears to be unstable and has numerical oscillations. However, when the CFL number is slightly below 1 (s = 0.999), the solution appears to be stable and accurately catches the behavior of the wave equation.
The product of the CFL number (s = c*dt/dx, where c is the wave speed) and the maximum eigenvalue of the numerical scheme must be less than or equal to 1. 
If the CFL number is greater than 1, the numerical scheme becomes unstable and numerical oscillations would occur. """
